...
def repeat_to_batch_size(t: torch.Tensor, target_batch: int) -> torch.Tensor:
    batch = t.shape[0]
    if batch == target_batch:
        return t
    if batch <= 0:
        raise ValueError("Cannot expand an empty batch.")

    repeats = math.ceil(target_batch / batch)
    repeat_shape = [repeats] + [1] * (len(t.shape) - 1)
    return t.repeat(*repeat_shape)[:target_batch]
...
class PasteByMask:
    ...
    def paste(self, image_base, image_to_paste, mask, resize_behavior, mask_mapping_optional = None):
        ...
        if mask_mapping_optional is None:
            target_batch = max(B, MB, PB)
            image_base = repeat_to_batch_size(image_base, target_batch)
            mask = repeat_to_batch_size(mask, target_batch)
            image_to_paste = repeat_to_batch_size(image_to_paste, target_batch)
        elif PB < MB:
            image_to_paste = repeat_to_batch_size(image_to_paste, MB)
        mask = torch.nn.functional.interpolate(mask.unsqueeze(1), size=(H, W), mode='nearest')[:,0,:,:]
        MB, MH, MW = mask.shape
